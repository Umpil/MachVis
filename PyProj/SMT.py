...
def DeleteNoNeigh(img, times=1):
    image = img.copy()
    ret_image = image.copy()
    for _ in range(times):
        for i in range(1, ret_image.shape[0] - 1):
            for j in range(1, ret_image.shape[1] - 1):
                if image[i][j] != 0:
                    if image[i-1][j] == 0 and image[i+1][j] == 0 and image[i][j+1] == 0:
                        ret_image[i][j] = 0
                    elif image[i][j+1] == 0 and image[i+1][j] == 0 and image[i][j-1] == 0:
                        ret_image[i][j] = 0
                    elif image[i-1][j] == 0 and image[i+1][j] == 0 and image[i][j+1] == 0:
                        ret_image[i][j] = 0
                    elif image[i-1][j] == 0 and image[i][j+1] == 0 and image[i][j-1] == 0:
                        ret_image[i][j] = 0
        image = ret_image.copy()

    return ret_image
def Canny(image, weak=None, strong=None, retStrong=True, ret255=True):
...
def ConnectLeftRight(image):
    ret_im = image.copy()
    for i in range(image.shape[0]):
        for j in range(1, image.shape[1] - 1):
            if image[i][j] == 0 and image[i][j-1] !=0 and image[i][j+1] !=0:
                ret_im[i][j] = 255
